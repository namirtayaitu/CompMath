...
def gauss_seidel_method(A, b, x0=None, epsilon=0.000001, max_iterations=100):
    """
    Solves Ax = b using Gauss-Seidel Iteration Method.
    ...
    for iteration in range(max_iterations):
        ...
        for i in range(n):
            s1 = sum(A[i][j] * x[j] for j in range(i))  # Use NEW values (j < i)
            s2 = sum(A[i][j] * x_old[j] for j in range(i + 1, n))  # Use OLD values (j > i)
            x_new_i = (b[i] - s1 - s2) / A[i, i]

            delta_i = abs(x_new_i - x[i])
...
print("Solving using Gauss-Seidel Iteration Method")
...
try:
    solution, iters = gauss_seidel_method(A, b, x0=[0, 0, 0, 0], epsilon=0.000001, max_iterations=100)
